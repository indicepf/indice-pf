...
// Sem fallback hardcoded de chave: as envs existem no Vercel e no .env.local;
// faltando, falha alto na carga (uma chave literal aqui mascara erro de config
// e apontaria para um projeto fixo mesmo trocando as envs).
const SUPABASE_URL = process.env.NEXT_PUBLIC_SUPABASE_URL || 'https://yhgdlmmtiyvdgeoxavzn.supabase.co'
const SUPABASE_ANON_KEY =
  process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ||
  process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY ||
  ''
if (!SUPABASE_ANON_KEY) throw new Error('NEXT_PUBLIC_SUPABASE_ANON_KEY não configurada')
