  ...
  return (
    <div className="space-y-5">
      ...
      {rows === null ? <p className="text-sm text-muted">Carregando…</p>
        : !rows.length ? <p className="text-sm text-muted text-center py-8">Nenhuma ação para este filtro.</p>
        : (
                  ...
                  {ab && (
                    <div className="px-3 pb-3 border-t border-line/60 pt-2 space-y-2">
                      <p className="text-xs text-muted">
                        ...
                        {link && <> · <a href={link} target="_blank" rel="noopener noreferrer" className="text-paprika hover:underline">local</a></>}
                      </p>
                      {!campos.length ? <p className="text-xs text-muted">Sem dados registrados.</p> : (
