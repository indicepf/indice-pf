...
export default function InfoTip({ texto, w = 'w-60', pos = 'center' }: { texto: string; w?: string; pos?: 'left' | 'center' | 'right' }) {
  ...
  return (
    <span className="relative inline-flex group align-middle ml-1">
      ...
      <span role="tooltip"
        className={`pointer-events-none absolute ${posCls} bottom-full mb-1.5 ${w} bg-ink text-cream text-[0.72rem] leading-snug rounded-md px-2.5 py-2 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity z-50 shadow-lg font-[family-name:var(--font-sans)] normal-case tracking-normal`}>
        {texto}
      </span>
    </span>
  )
